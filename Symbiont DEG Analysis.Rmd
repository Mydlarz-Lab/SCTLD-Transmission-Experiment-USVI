...
setwd("./SymbA/salmon/")
cladeA_samples <- read.table("SymbA_samples.csv", header = TRUE)
...
CladeA_tx2gene <- read.csv("SymbA_tx2gene.csv")
...
```{r,results='hide',tidy=TRUE}
cladeA_countData <- read.csv("./SymbA/salmon/cladeA_counts.csv", row.names="Entry")
colData <- read.csv("./../metaData.csv", row.names = "name")
cladeAColData <- colData[colData$dominant_clade == 'A',]
cladeAColData <- cladeAColData[-c(12),]
colnames(cladeA_countData)
...
write.csv(cladeA_res, file = "./SymbA/DESeq2/DESeq_treatment.csv")

rld <- rlog(dds, blind=FALSE) 
cladeA_rlogs <- as.data.frame(assay(rld))
write.csv(cladeA_rlogs, file = "./SymbA/DESeq2/treatment_rlog.csv")  
...
```{r,results='hide',tidy=TRUE}
cladeB_countData <- read.csv("./SymbB/salmon/cladeB_counts.csv", row.names="Entry")
colData <- read.csv("./../metaData.csv", row.names = "name")
...
write.csv(cladeB_res, file = "./SymbB/DESeq2/DESeq_treatment.csv")

rld <- rlog(dds, blind=FALSE) 
cladeB_rlogs <- as.data.frame(assay(rld))
write.csv(cladeB_rlogs, file = "./SymbB/DESeq2/treatment_rlog.csv")  

# Keep significant DEGs (padj < 0.05)
DEGs <- cladeB_res %>% filter(padj < 0.05)
write.csv(DEGs, file = "./SymbB/DESeq2/treatment_sig_DEGs.csv")  
print(DEGs)
```
...
```{r,results='hide',tidy=TRUE}
cladeC_countData <- read.csv("./SymbC/salmon/cladeC_counts.csv", row.names="Entry")
colData <- read.csv("./../metaData.csv", row.names = "name")
...
write.csv(cladeC_res, file = "./SymbC/DESeq2/DESeq_treatment.csv")

rld <- rlog(dds, blind=FALSE) 
cladeC_rlogs <- as.data.frame(assay(rld))
write.csv(cladeC_rlogs, file = "./SymbC/DESeq2/treatment_rlog.csv")  
...
```{r,results='hide',tidy=TRUE}
setwd("./SymbC/salmon")
cladeC_samples <- read.csv("SymbC_samples_no_outlier.csv")
cladeC_files <- file.path(cladeC_samples$sample, "quant.sf")
...
# Keep significant DEGs (padj < 0.05)
DEGs <- cladeC_res %>% filter(padj < 0.05)
write.csv(DEGs, file = "./SymbC/DESeq2/treatment_sig_DEGs.csv")  
print(DEGs)
```
...
```{r,results='hide',tidy=TRUE}
cladeD_countData <- read.csv("./SymbD/salmon/cladeD_counts.csv", row.names="Entry")
colData <- read.csv("./../metaData.csv", row.names = "name")
...
write.csv(cladeD_res, file = "./SymbD/DESeq2/DESeq_treatment.csv")

rld <- rlog(dds, blind=FALSE) 
cladeD_rlogs <- as.data.frame(assay(rld))
write.csv(cladeD_rlogs, file = "./SymbD/DESeq2/treatment_rlog.csv")  

# Keep significant DEGs (padj < 0.05)
DEGs <- cladeD_res %>% filter(padj < 0.05)
write.csv(DEGs, file = "./SymbD/DESeq2/treatment_sig_DEGs.csv")  
print(DEGs)
```
...
DEGs <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Experiment/symbiont/SymbB/DESeq2/treatment_sig_DEGs.csv")
...
cladeB_DEGs_annot <- merge(DEGs,uniprot,by="Entry",all=FALSE)
rlogs <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Experiment/symbiont/SymbB/DESeq2/treatment_rlog.csv")
colnames(rlogs)[colnames(rlogs)=="X"] <- "Entry"
cladeB_DEGs_rlog <- merge(cladeB_DEGs_annot,rlogs,by="Entry",all=FALSE)
names(cladeB_DEGs_rlog)
cladeB_DEGs_rlog <- tibble::column_to_rownames(cladeB_DEGs_rlog,"Protein.names")
cladeB_DEGs_rlog <- cladeB_DEGs_rlog[,-c(1:11)]

label <- read.csv("./../metaData.csv", row.names = "name")
label <- label[label$dominant_clade == 'B',]
colnames(label)
label <- label[,-c(2,3,5,7:21)]

breaks = seq(-2,2, by=0.1)
pheatmap(cladeB_DEGs_rlog,
         cluster_cols = FALSE,
         cluster_rows = TRUE,
         scale = "row", # plots relative expression instead of rlog expression
         border_color = "black",
         ...
         cellheight = 12,
         main = "Breviolum DEGs",
         color = colorRampPalette(c("#4393c3","white","#d6604d"))(length(breaks)),
         legend = TRUE)
```

### Clade C
```{r,fig.height=5,fig.width=25}
DEGs <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Experiment/symbiont/SymbC/DESeq2/treatment_sig_DEGs.csv")
colnames(DEGs)[colnames(DEGs)=="X"] <- "Entry"
uniprot <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Files_for_R/uniprot.csv")
cladeC_DEGs_annot <- merge(DEGs,uniprot,by="Entry",all=FALSE)
rlogs <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Experiment/symbiont/SymbC/DESeq2/treatment_rlog.csv")
colnames(rlogs)[colnames(rlogs)=="X"] <- "Entry"
cladeC_DEGs_rlog <- merge(cladeC_DEGs_annot,rlogs,by="Entry",all=FALSE)
names(cladeC_DEGs_rlog)
cladeC_DEGs_rlog <- tibble::column_to_rownames(cladeC_DEGs_rlog,"Protein.names")
cladeC_DEGs_rlog <- cladeC_DEGs_rlog[,-c(1:11)]

label <- read.csv("./../metaData.csv", row.names = "name")
label <- label[label$dominant_clade == 'C',]
colnames(label)
label <- label[,-c(2,3,5,7:21)]
rownames(label)
label <- label[-c(19),]
...
### Clade D
```{r,fig.height=5,fig.width=25}
DEGs <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Experiment/symbiont/SymbD/DESeq2/treatment_sig_DEGs.csv")
colnames(DEGs)[colnames(DEGs)=="X"] <- "Entry"
uniprot <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Files_for_R/uniprot.csv")
cladeD_DEGs_annot <- merge(DEGs,uniprot,by="Entry",all=FALSE)
rlogs <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Experiment/symbiont/SymbD/DESeq2/treatment_rlog.csv")
colnames(rlogs)[colnames(rlogs)=="X"] <- "Entry"
cladeD_DEGs_rlog <- merge(cladeD_DEGs_annot,rlogs,by="Entry",all=FALSE)
names(cladeD_DEGs_rlog)
cladeD_DEGs_rlog <- tibble::column_to_rownames(cladeD_DEGs_rlog,"Protein.names")
cladeD_DEGs_rlog <- cladeD_DEGs_rlog[,-c(1:11)]

label <- read.csv("./../metaData.csv", row.names = "name")
label <- label[label$dominant_clade == 'D',]
colnames(label)
label <- label[,-c(2,3,5,7:21)]

breaks = seq(-2,2, by=0.1)
pheatmap(cladeD_DEGs_rlog,
         cluster_cols = FALSE,
         cluster_rows = TRUE,
         scale = "row", # plots relative expression instead of rlog expression
         border_color = "black",
...
### Clade A
```{r}
coralShared_rlogs <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Files_for_R/January_2022/SCTLD/host/all/DESeq2/treatment_rlog.csv",row.names = "X")
rab7 <- coralShared_rlogs[c("H9BW96"),]
names(rab7)
t_rab7 <- as.data.frame(t(rab7))
t_rab7 <- tibble::rownames_to_column(t_rab7, "name")

cladeA_rlog <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Files_for_R/January_2022/SCTLD/symbiont/cladeA/DESeq2/treatment_rlog.csv",row.names = "X")
names(cladeA_rlog)
curatedGenes <- cladeA_rlog[c("Q9XQV3","P49471","P49472","Q59094","Q5BCC5","Q96NN9","P11144"),]
...
cladeA_correlations <- merge(t_rab7,t_rlogs,by="name")
...
colnames(cladeA_correlations)[2] <- "psaA"
colnames(cladeA_correlations)[3] <- "psbB"
colnames(cladeA_correlations)[4] <- "psbC"
colnames(cladeA_correlations)[5] <- "Superoxide dismutase"
colnames(cladeA_correlations)[6] <- "Carbonic anhydrase"
colnames(cladeA_correlations)[7] <- "Apoptosis-inducing factor 3"
colnames(cladeA_correlations)[8] <- "Heat shock protein 70 "
colnames(cladeA_correlations)

A = cor(cladeA_correlations)
A.testRes = cor.mtest(cladeA_correlations, conf.level = 0.95)

Symbiodinium_pvalues <- as.data.frame(A.testRes$p)
Symbiodinium_R <- as.data.frame(A)
Symbiodinium_R
...
cladeB_rlog <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Files_for_R/January_2022/SCTLD/symbiont/cladeB/DESeq2/treatment_rlog.csv",row.names = "X")
names(cladeB_rlog)
curatedGenes <- cladeB_rlog[c("Q9XQV3","P49471","P49472","Q59094","Q5BCC5","Q96NN9","P11144"),]
t_rlogs <- as.data.frame(t(curatedGenes))
t_rlogs <- tibble::rownames_to_column(t_rlogs, "name")
cladeB_correlations <- merge(t_rab7,t_rlogs,by="name")
...
colnames(cladeB_correlations)[2] <- "psaA"
colnames(cladeB_correlations)[3] <- "psbB"
colnames(cladeB_correlations)[4] <- "psbC"
colnames(cladeB_correlations)[5] <- "Superoxide dismutase"
colnames(cladeB_correlations)[6] <- "Carbonic anhydrase"
colnames(cladeB_correlations)[7] <- "Apoptosis-inducing factor 3"
colnames(cladeB_correlations)[8] <- "Heat shock protein 70 "
colnames(cladeB_correlations)

B = cor(cladeB_correlations)
B.testRes = cor.mtest(cladeB_correlations, conf.level = 0.95)
Breviolum_pvalues <- as.data.frame(B.testRes$p)
Breviolum_R <- as.data.frame(B)
Breviolum_R
...
cladeC_rlog <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Files_for_R/January_2022/SCTLD/symbiont/cladeC/DESeq2/treatment_rlog.csv",row.names = "X")
names(cladeC_rlog)
curatedGenes <- cladeC_rlog[c("Q9XQV3","P49471","P49472","Q59094","Q5BCC5","Q96NN9","P11144"),]
t_rlogs <- as.data.frame(t(curatedGenes))
t_rlogs <- tibble::rownames_to_column(t_rlogs, "name")
cladeC_correlations <- merge(t_rab7,t_rlogs,by="name")
...
colnames(cladeC_correlations)[2] <- "psaA"
colnames(cladeC_correlations)[3] <- "psbB"
colnames(cladeC_correlations)[4] <- "psbC"
colnames(cladeC_correlations)[5] <- "Superoxide dismutase"
colnames(cladeC_correlations)[6] <- "Carbonic anhydrase"
colnames(cladeC_correlations)[7] <- "Apoptosis-inducing factor 3"
colnames(cladeC_correlations)[8] <- "Heat shock protein 70 "
cladeC_correlations <- cladeC_correlations[,-c(2)]
colnames(cladeC_correlations)

C = cor(cladeC_correlations)
C.testRes = cor.mtest(cladeC_correlations, conf.level = 0.95)
Cladocopium_pvalues <- as.data.frame(C.testRes$p)
Cladocopium_pvalues
Cladocopium_R <- as.data.frame(C)
Cladocopium_R
...
cladeD_rlog <- read.csv("~/OneDrive - University of Texas at Arlington/Dissertation/SCTLD/Files_for_R/January_2022/SCTLD/symbiont/cladeD/DESeq2/treatment_rlog.csv",row.names = "X")
names(cladeD_rlog)
curatedGenes <- cladeD_rlog[c("Q9XQV3","P49471","P49472","Q59094","Q5BCC5","Q96NN9","P11144"),]
t_rlogs <- as.data.frame(t(curatedGenes))
t_rlogs <- tibble::rownames_to_column(t_rlogs, "name")
cladeD_correlations <- merge(t_rab7,t_rlogs,by="name")
...
colnames(cladeD_correlations)[2] <- "psaA"
colnames(cladeD_correlations)[3] <- "psbB"
colnames(cladeD_correlations)[4] <- "psbC"
colnames(cladeD_correlations)[5] <- "Superoxide dismutase"
colnames(cladeD_correlations)[6] <- "Carbonic anhydrase"
colnames(cladeD_correlations)[7] <- "Apoptosis-inducing factor 3"
colnames(cladeD_correlations)[8] <- "Heat shock protein 70 "
colnames(cladeD_correlations)

D = cor(cladeD_correlations)
D.testRes = cor.mtest(cladeD_correlations, conf.level = 0.95)
Durusdinium_pvalues <- as.data.frame(D.testRes$p)
Durusdinium_pvalues
Durusdinium_R <- as.data.frame(D)
Durusdinium_R
